  ...
  public class MessageRepository : IMessageRepository
  {
    ...
    public async Task<ClasificacionDeterminadaDto> ClasificarMensaje(string message)
    {
      ...
      try
      {
        ...
        string textoProcesado = PreprocesarTexto(message);
        float[] vector = VectorizarTexto(textoProcesado);
    ...
    static string PreprocesarTexto(string texto)
    {
      // 1. Lowercase
      texto = texto.ToLower();
    ...
    static float[] VectorizarTexto(string textoProcesado)
    {
      var vocabData = JsonSerializer.Deserialize<VocabularioData>(
    File.ReadAllText("vocabulario.json")
);
