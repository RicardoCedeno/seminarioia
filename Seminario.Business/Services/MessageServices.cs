...
namespace Seminario.Business.Services
{
  public class MessageServices(IMessageRepository messageRepository) : IMessageServices
  {
    ...
    public async Task<Message> SendMessage(Message message)
    {
      ...
      if (sentimientoClasificado.Formatos.Contains("solicitud") || (mensajeClasificado.Formatos != null && mensajeClasificado.Formatos.Any()))
      {
      ...
      if (sentimientoClasificado.Formatos.Count == 1 && sentimientoClasificado.Formatos.First() == "saludo" && mensajeClasificado.Formatos.Count == 0)
      {
      ...
      {
        rta.SentMessage.Add("Por favor, vuelve a realizar la pregunta. Si no recibes la información requerida puedes enviar un mensaje a PQR para resolver tu duda");
      }

      if (sentimientoClasificado.Formatos.Contains("enojo") && mensajeClasificado.Formatos != null && mensajeClasificado.Formatos.Contains("solicitud"))
      {
        rta.SentMessage = [];
        rta.SentMessage.Add($"Tienes razón. Muy buena observación, te di los formatos equivocados, aquí tienes los formatos de {string.Join(",", mensajeClasificado.Formatos)}");
      }
